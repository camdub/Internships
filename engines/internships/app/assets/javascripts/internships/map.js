...
function setupDialogBox(id){
	...
	if($('#dialog-'+id).html() == null){
		$.ajax({
			url: '/internships/internships/' + id + '.json',
		  	dataType: 'json',
		  	success: function(data){
					var html = new EJS({ url: '/assets/internships/templates/modal_view.ejs'}).render(data);
					$('#dialogs').append(html);
...
function initList(){
	...
	var html = "";
  if(internship_data.countries.USA != undefined){
    html = new EJS({url: '/assets/internships/templates/list_view.ejs'}).render(internship_data);
  }
...
function getInternshipData(){
	$.ajax({
		url: '/internships/internships.json',
		data: filters,
		dataType: 'json',
		success: function(data){
...
function getMapData(){
	$("#svg-map").svg('destroy');
	$("#svg-map").svg({
		loadURL: '/assets/internships/map.svg',
		onLoad: function(){
			mapIsLoaded = true;
		}
	});
